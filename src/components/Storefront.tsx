...
export default function Storefront({ onTrace }: StorefrontProps) {
  ...
  const handleOpenQr = useCallback((id: string) => {
    setQrModal(id);
  }, []);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {displayedProducts.map((product, i) => (
          <VirtualItem key={product.id} estimateHeight={450}>
            <ProductCard
              product={product}
              tmpl={templateByBlockchainId[product.id]}
              i={i}
              onTrack={handleTrack}
      ...
      {qrModal && (() => {
        ...
        return (
          <div className="gpu-accelerated fixed inset-0 z-[200] flex items-center justify-center p-4 bg-[var(--overlay-bg)] backdrop-blur-sm"
            onClick={() => setQrModal(null)}>
            <div className="glass-card gpu-accelerated p-8 text-center max-w-sm" onClick={e => e.stopPropagation()} style={{ borderRadius: '1.5rem' }}>
